...
type AppMode = "safety" | "formation" | "assetUse";
type AssetUseTab = "timeBucket" | "quickTrial" | "review" | "incomePower";
type PrimaryNavKey = "dashboard" | "safety" | "formation" | "assetUse" | "results" | "compare" | "data";
...
const ONBOARDING_COMPLETED_KEY = "life-asset-simulator-beta-onboarding-completed-v1";
const TRUST_EXPLAINED_KEY = "life-asset-simulator-beta-trust-explained-v1";
...
const primarySectionClassNames: Record<PrimaryNavKey, { active: string; inactive: string }> = {
  ...
  formation: {
    active: "border border-emerald-800 bg-emerald-800 text-white shadow-sm hover:bg-emerald-900",
    inactive: "border border-emerald-200 bg-emerald-50 text-emerald-900 hover:bg-emerald-100",
  },
  results: {
...
function appModeFromHash(): AppMode {
  if (window.location.hash === "#/formation") return "formation";
  return window.location.hash === "#/asset-use" ? "assetUse" : "safety";
}

function setAppModeHash(mode: AppMode) {
  const nextHash = mode === "assetUse" ? "#/asset-use" : mode === "formation" ? "#/formation" : "";
...
function App() {
  ...
  const exportJson = () => {
    ...
    downloadText("life-asset-simulator-beta.json", JSON.stringify(state, null, 2));
  };
  ...
  const createBackupAndExport = () => {
    ...
    downloadText(`life-asset-simulator-beta-backup-${timestamp}.json`, JSON.stringify(state, null, 2));
  };
    ...
    <div className="min-h-screen">
      ...
      <header className="border-b bg-white/90 backdrop-blur">
        <div className="container flex flex-col gap-4 py-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm text-muted-foreground">β版</p>
            <h1 className="text-2xl font-semibold tracking-normal">人生資産シミュレーション β</h1>
          </div>
        ...
        <nav className="container flex gap-2 overflow-x-auto pb-2">
          ...
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setAppModeHash("formation")}
            className={primaryNavClass("formation")}
          >
            資産形成
          </Button>
          <Button
...
function AssetFormationDashboard({
  scenario,
  result,
  updateScenario,
  onOpenProfile,
  ...
  const targetRow = result.annual.find((row) => row.ageYears >= targetAge) ?? result.annual.at(-1);
  const startRow = result.monthly[0];
  const currentAgeMonths = startRow ? startRow.ageYears * 12 + startRow.ageMonths : 0;
  const targetAgeMonths = targetRow ? targetRow.ageYears * 12 + targetRow.ageMonths : currentAgeMonths;
  const monthsToTarget = Math.max(1, targetAgeMonths - currentAgeMonths);
  const projectedTargetBalance = targetRow?.endingAssets ?? 0;
  const targetGap = targetAmount > 0 ? targetAmount - projectedTargetBalance : 0;
  const additionalMonthlyRequired = targetGap > 0 ? Math.ceil(targetGap / monthsToTarget / 1000) * 1000 : 0;
  const reachRow = targetAmount > 0 ? result.annual.find((row) => row.endingAssets >= targetAmount) : undefined;
  const activeContributionEvents = scenario.assetContributionEvents.filter((event) => isContributionActive(event, startMonth));
  const activeMonthlyContribution = activeContributionEvents.reduce((sum, event) => sum + event.monthlyAmount, 0);
  const totalContribution = result.annual.reduce((sum, row) => sum + row.assetContributionTotal, 0);
  const monthlyLiving = getBaseMonthlyExpense(scenario.monthlyExpenses, true);
  const sixMonthReserve = monthlyLiving * 6;
  ...
  const chartData = result.annual.map((row) => ({
    ...
    target: targetAmount > 0 ? Math.round(targetAmount / 10_000) : undefined,
  }));
    ...
    <div className="space-y-6">
      <section className="rounded-lg border bg-white p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-sm text-emerald-700">β版の追加機能</p>
            <h2 className="mt-1 text-xl font-semibold">資産形成ダッシュボード</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
              目標資産に届くか、今の積立でどこまで伸びるか、生活防衛資金が足りているかを先に見ます。
              退職後の取り崩し計算は既存タブに残し、ここでは形成期の判断に絞ります。
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button variant="outline" onClick={onOpenProfile}>目標を直す</Button>
            <Button variant="outline" onClick={onOpenAssets}>積立を直す</Button>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-4">
        <Metric
          title={`${targetAge}歳時点の見込み資産`}
          value={compactYen(projectedTargetBalance)}
          sub={targetAmount > 0 ? `目標 ${compactYen(targetAmount)} / 差額 ${compactYen(projectedTargetBalance - targetAmount)}` : "目標資産を入れると差額を表示します"}
        />
        <Metric
          title="目標到達見込み"
          value={reachRow ? `${reachRow.ageYears}歳` : targetAmount > 0 ? "未達" : "-"}
          sub={reachRow ? `${reachRow.year}年末に目標を上回る見込みです` : "目標年齢・目標資産を確認してください"}
        />
        <Metric
          title="必要な追加積立"
          value={additionalMonthlyRequired > 0 ? `${yen(additionalMonthlyRequired)}/月` : "追加不要"}
          sub="不足額を目標月まで単純割りした目安です"
        />
        <Metric
          title="生活防衛資金"
          value={reserveGap > 0 ? `${compactYen(reserveGap)}不足` : "目安クリア"}
          sub={`現金・預金 ${compactYen(liquidAssets)} / 目安 ${compactYen(reserveTarget)}`}
        />
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
        <Card>
          <CardHeader>
            <CardTitle>資産形成の見通し</CardTitle>
            <CardDescription>年末資産と目標資産の差を確認します。単位は万円です。</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={{ top: 8, right: 18, bottom: 24, left: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" />
                  <XAxis dataKey="age" interval="preserveStartEnd" />
                  <YAxis tickFormatter={(value) => `${value}万`} width={72} />
                  <Tooltip formatter={(value, name) => [`${Number(value).toLocaleString()}万円`, name === "assets" ? "年末資産" : "目標資産"]} />
                  <Legend />
                  <Line type="monotone" dataKey="assets" name="年末資産" stroke="#047857" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="target" name="目標資産" stroke="#dc2626" strokeDasharray="6 4" dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>形成期の前提</CardTitle>
            <CardDescription>まずは目標と現在の積立を粗く確認します。</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <FormGrid>
              <Field label="目標年齢">
                <Input
                  type="number"
                  value={targetAge}
                  onChange={(event) => updateScenario((s) => void (s.userProfile.targetBalanceAge = Math.max(1, numberOrZero(event.target.value))))}
                />
              </Field>
              <Field label="目標資産">
                <Input
            ...
            <div className="rounded-lg border bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-700">
              現在有効な毎月積立は {yen(activeMonthlyContribution)}、試算期間全体の追加投資合計は {compactYen(totalContribution)} です。
              積立先や終了年月は「初期資産」タブの毎月の追加投資で編集します。
            </div>
          </CardContent>
        </Card>
      </section>
