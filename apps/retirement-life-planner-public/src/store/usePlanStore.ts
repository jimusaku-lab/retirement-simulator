...
function normalizeOptionSubAccounts(source: LegacyScenario, legacyAssets: LegacyInitialAssets): OptionSubAccount[] {
  ...
  if (Array.isArray(existing) && existing.length > 0) {
    return existing.map((account, index) => ({
      ...
      name: (account.name ?? `一般口座${index + 1}`).replaceAll("普通口座", "一般口座"),
...
function isLegacyPrivateStarterSample(scenarios: ScenarioData[]) {
  ...
  return legacyPrivateStarterSampleIds.every((id) => scenarios.some((scenario) => scenario.id === id));
}

function alignPublicStarterSampleFlexiblePeriod(scenarios: ScenarioData[]) {
  const sampleById = new Map(sampleState.scenarios.map((scenario) => [scenario.id, scenario]));
  for (const scenario of scenarios) {
    const sample = sampleById.get(scenario.id);
    if (!sample || scenario.name !== sample.name) continue;
    const isOldPublicDefault =
      scenario.userProfile.birthDate === sample.userProfile.birthDate &&
      scenario.userProfile.simulationStartYearMonth === sample.userProfile.simulationStartYearMonth &&
      scenario.userProfile.flexibleFreeCashStartAge === 60 &&
      scenario.userProfile.flexibleFreeCashEndAge === sample.userProfile.flexibleFreeCashEndAge;
    if (!isOldPublicDefault) continue;
    scenario.userProfile.flexibleFreeCashStartAge = sample.userProfile.flexibleFreeCashStartAge;
  }
}

export function normalizePlanState(input: Partial<RetirementPlanState> | undefined): RetirementPlanState {
  let scenarios = Array.isArray(input?.scenarios) && input.scenarios.length > 0
  ...
  const publicSampleHasPensionCases =
    ...
    scenarios.some((scenario) => scenario.id === "pension-later");
  if (publicSampleHasPensionCases && !scenarios.some((scenario) => scenario.id === "pension-earlier")) {
    const pensionEarlier = sampleState.scenarios.find((scenario) => scenario.id === "pension-earlier");
    if (pensionEarlier) {
      const insertIndex = scenarios.findIndex((scenario) => scenario.id === "pension-later");
      scenarios.splice(insertIndex >= 0 ? insertIndex : scenarios.length, 0, structuredClone(pensionEarlier));
    }
  }
  alignPublicStarterSampleFlexiblePeriod(scenarios);
...
export const usePlanStore = create<PlanStore>()(
  persist(
    ...
    {
      name: "retirement-life-planner-public-v1",
