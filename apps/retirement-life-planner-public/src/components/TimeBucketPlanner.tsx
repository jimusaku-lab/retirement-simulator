...
function bucketMemberAgeLabels(bucketId: TimeBucketBucketId, scenario: ScenarioData) {
  ...
  const bucket = createRetirementBuckets(scenario).find((item) => item.id === bucketId);
  if (bucket?.startAge !== undefined) {
    const bucketEndAge = bucket.endAge ?? bucket.startAge;
    return scenario.householdMembers
      .filter((member) => member.relationship === "self" || member.relationship === "spouse")
      .filter((member) => {
        const memberAge = ageAtYearMonth(member.birthDate, scenario.userProfile.simulationStartYearMonth);
        return memberAge !== undefined && memberAge >= bucket.startAge! && memberAge <= bucketEndAge;
      })
...
function createRetirementBuckets(scenario: ScenarioData): TimeBucketBucket[] {
  ...
  const horizonEndAge = Math.max(period.endAge, scenario.userProfile.simulationEndAge ?? period.endAge, scenario.userProfile.targetBalanceAge ?? period.endAge);
  ...
  let toneIndex = 0;
  for (let startAge = period.startAge; startAge <= horizonEndAge;) {
    const endAge = Math.min(
      horizonEndAge,
      startAge < 60 ? 59 : startAge <= 75 ? startAge + 1 : startAge + 9,
    );
  ...
  const laterAge = horizonEndAge + 1;
  buckets.push({
...
const wishTemplates: Array<{ title: string; bucketId: TimeBucketBucketId; note: string }> = [
  { title: "国内旅行を増やす", bucketId: "age-60-61", note: "金額が決まったら、毎年または単発の楽しみ支出にします。" },
  { title: "夫婦で長めの旅行", bucketId: "age-62-63", note: "健康寿命期の楽しみ支出候補です。" },
  { title: "趣味・学びを始める", bucketId: "age-60-61", note: "月額または年額が決まってから支出化します。" },
  { title: "家族イベントを支援する", bucketId: "age-64-65", note: "家族支援カテゴリの候補です。" },
  { title: "住まいの修繕を考える", bucketId: "age-66-67", note: "住宅・車カテゴリの候補です。" },
  { title: "医療・介護の備えを置く", bucketId: "age-72-73", note: "医療・介護カテゴリの候補です。" },
];
...
export function TimeBucketPlanner({ scenario, scenarios, updateScenario, updateScenarios, onOpenSpecialExpenses }: TimeBucketPlannerProps) {
  ...
  const linkedSpecialExpenses = scenario.timeBucketItems
    .map((item) => scenario.specialExpenses.find((event) => event.id === item.convertedSpecialExpenseId))
    .filter((event): event is SpecialExpenseEvent => Boolean(event));
  const linkedSpecialExpenseTotal = linkedSpecialExpenses.reduce((sum, event) => sum + event.amount, 0);
  const convertedItemCount = linkedSpecialExpenses.length;
  const unconvertedItemCount = scenario.timeBucketItems.filter((item) => !item.convertedSpecialExpenseId).length;
  const enjoymentLinkedTotal = linkedSpecialExpenses
  ...
  const addTemplateItem = (template: (typeof wishTemplates)[number]) => {
    const visibleBucketIds = new Set(visibleBuckets.map((bucket) => bucket.id));
    updateScenario((draft) => {
      draft.timeBucketItems.unshift({
        id: createId(),
        title: template.title,
        bucketId: visibleBucketIds.has(template.bucketId) ? template.bucketId : "todo",
        note: template.note,
      });
    });
    setOpenItemActionId(null);
  ...
  const removeItem = (itemId: string) => {
    ...
    setOpenItemActionId((current) => current === itemId ? null : current);
  };
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>やりたいこと整理</CardTitle>
          <CardDescription>金額未定のまま仮置きし、計算に入れるものだけ特別支出へ変換します。</CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="grid gap-3 md:grid-cols-4">
            <div className="rounded-md border bg-emerald-50 px-4 py-3">
              <div className="text-sm font-medium text-emerald-900">支出化済み</div>
              <div className="mt-1 text-2xl font-semibold text-emerald-950">{convertedItemCount}件</div>
              <p className="mt-1 text-xs leading-5 text-emerald-900">計算に入っているやりたいことです。</p>
            </div>
            <div className="rounded-md border bg-slate-50 px-4 py-3">
              <div className="text-sm font-medium text-slate-700">金額未定・未支出化</div>
              <div className="mt-1 text-2xl font-semibold text-slate-950">{unconvertedItemCount}件</div>
              <p className="mt-1 text-xs leading-5 text-slate-600">整理だけの状態なので、試算結果には入りません。</p>
            </div>
            <div className="rounded-md border bg-rose-50 px-4 py-3">
              <div className="text-sm font-medium text-rose-900">楽しみ支出</div>
              <div className="mt-1 text-2xl font-semibold text-rose-950">{compactYen(enjoymentLinkedTotal)}</div>
              <p className="mt-1 text-xs leading-5 text-rose-900">旅行・趣味などの支出化済み合計です。</p>
            </div>
            <div className="rounded-md border bg-blue-50 px-4 py-3">
              <div className="text-sm font-medium text-blue-900">支出化済み合計</div>
              <div className="mt-1 text-2xl font-semibold text-blue-950">{compactYen(linkedSpecialExpenseTotal)}</div>
              <p className="mt-1 text-xs leading-5 text-blue-900">カテゴリ全体の計算対象額です。</p>
            </div>
          </div>

          <div className="rounded-lg border bg-white px-4 py-3">
            <div className="text-sm font-medium">テンプレートから追加</div>
            <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {wishTemplates.map((template) => (
                <Button key={template.title} type="button" variant="outline" className="justify-start" onClick={() => addTemplateItem(template)}>
            ...
            <p className="mt-3 text-xs leading-6 text-muted-foreground">
              ここで追加しただけでは計算に入りません。金額と時期が決まったものだけ、カードの「支出へ」から特別支出に変換します。
            </p>
          </div>
        ...
        {visibleBuckets.map((bucket) => {
          ...
          return (
            <section
              ...
              className={[
                "flex min-h-0 flex-col rounded-lg border bg-white p-4 shadow-sm transition",
            ...
            >
              ...
              <div
                className={[
                  "mt-4 gap-3",
                  isTodoBucket ? "grid grid-flow-col grid-rows-[repeat(5,max-content)] auto-cols-[minmax(14rem,1fr)] items-start" : "flex flex-col",
                ].join(" ")}
              >
                {bucketItems.map((item) => {
                  ...
                  const hasSecondaryActions = converted || linkMissing;
                  const itemActionsOpen = openItemActionId === item.id;
                  return (
                    <article
                      key={item.id}
                      draggable
                    ...
                    >
                      <div className="grid grid-cols-[auto_1fr_auto_auto] items-start gap-2">
                        ...
                        {hasSecondaryActions ? (
                          <button
                            type="button"
                            onClick={() => setOpenItemActionId((current) => current === item.id ? null : item.id)}
                      ...
                      <div className="mt-3">
                        {converted && (
                          <span className="inline-flex rounded-full bg-emerald-50 px-2 py-1 text-xs font-medium leading-5 text-emerald-700">
                            支出化済み: {linkedSpecialExpense ? convertedSummary(linkedSpecialExpense) : ""}
                          </span>
                        )}
                        {linkMissing && (
                          <span className="inline-flex rounded-full bg-amber-50 px-2 py-1 text-xs font-medium leading-5 text-amber-700">
                            リンク先の特別支出が見つかりません
                          </span>
                        )}
                        {!converted && !linkMissing && (
                          bucket.kind === "hold" ? (
                            <span className="inline-flex rounded-full bg-slate-100 px-2 py-1 text-xs font-medium leading-5 text-slate-600">
                              支出化しない候補
                            </span>
                          ) : (
                      ...
                      {itemActionsOpen && (
                        <div className="mt-3 flex flex-wrap gap-2 rounded-md bg-slate-50 p-2">
                          {converted && (
                            <>
                ...
                {bucketItems.length === 0 && (
                  <div className="flex min-h-36 items-center justify-center rounded-md border border-dashed border-slate-200 px-3 py-8 text-center text-sm text-slate-400">
                    ここに移動
                  </div>
                )}
              </div>
